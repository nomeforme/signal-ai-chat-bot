...
class User:
    ...
    def increment_bot_interaction_counter(self):
        """Increment the bot interaction counter (when this bot interacts with another bot)"""
        self.bot_interaction_count += 1

    def reset_bot_interaction_counter(self):
        """Reset the bot interaction counter (called when human sends message)"""
        self.bot_interaction_count = 0

    def is_bot_loop_limit_reached(self):
        """Check if bot has interacted too many times with other bots"""
        return self.bot_interaction_count >= config.MAX_BOT_MENTIONS_PER_CONVERSATION
