...
if [ "$#" -ne 2 ]; then
    ...
    echo "Example: $0 +12186633092 \"CH45-natureterminal\""
    exit 1
fi
...
# Step 3: Update Signal profile via REST API
echo -e "${YELLOW}Step 3: Updating Signal profile name...${NC}"

RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT "${SIGNAL_API_URL}/v1/profiles/${PHONE_NUMBER}" \
    -H "Content-Type: application/json" \
    -d "{\"base64_avatar\": \"\", \"name\": \"$NEW_NAME\"}")
