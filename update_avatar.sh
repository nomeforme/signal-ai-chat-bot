...
if [ "$#" -ne 2 ]; then
    ...
    echo "Example: $0 +123456789 /path/to/avatar.jpg"
    exit 1
fi
...
CURRENT_PROFILE=$(curl -s "${SIGNAL_API_URL}/v1/profiles/${PHONE_NUMBER}")
CURRENT_NAME=$(echo "$CURRENT_PROFILE" | jq -r '.name' 2>/dev/null || echo "")

# If profile name is empty, try to get it from config.json
if [ -z "$CURRENT_NAME" ] || [ "$CURRENT_NAME" = "null" ]; then
    echo "  → Profile name not set in Signal, checking config.json..."
    ...
    if [ -f ".env" ]; then
        ...
        # Get bot name from config.json
        if [ $BOT_INDEX -ne -1 ] && [ -f "config.json" ]; then
            CURRENT_NAME=$(jq -r ".bots[$BOT_INDEX].name" config.json)
    ...
    echo "Current profile name: $CURRENT_NAME"
fi
echo ""
